                    ...
                    <div class='table-responsive tw-max-h-full no-scrollbar'>
                        <table class='tw-w-full tw-table-auto'>
                            ...
                            <tbody>
                                ...
                                $amountBalanceLast = 0;
                                @endphp
                                @forelse ($data->groupBy('nama_item') as $row)
                                <tr>
                                    <td class="tw-text-sm tw-tracking-wider" colspan="10">
                                        <b>Produk: {{ $row[0]->nama_item }}</b>
                                    </td>
                                </tr>
                                @foreach ($row->groupBy('tanggal') as $result)
                                <tr>
                                    <td class="tw-text-sm tw-tracking-wider tw-pl-10" colspan="10">
                                        <b>Tanggal: <span class="text-primary">{{ $result[0]->tanggal }}</span></b>
                                    </td>
                                </tr>
                                @foreach ($result as $results)
                                <tr class='text-center'>
                                    <td class='tw-p-3 tw-whitespace-nowrap text-left tw-pl-16'>
                                        {{ $results->keterangan }}</td>
