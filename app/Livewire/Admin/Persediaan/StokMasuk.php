...
class StokMasuk extends Component
{
    ...
    {
        $this->id_cabang = Auth::user()->id_cabang; // Set ID cabang berdasarkan user yang login
    ...
    {
        $this->isEditing        = true;
        $data = Persediaan::select('id', 'id_produk', 'tanggal', 'qty', 'keterangan')->where('id', $id)->first();
        $this->dataId           = $id;
